    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        public void Update()
        {
            ...
            if (Inventory.main != null)
            {
                ...
                if (heldtool != null)
                {
                    if (heldtool is Seaglide)
                    {
                        ...
                        if (timer == 100)
                        { 
                            Logger.LogInfo($"The storage containers for {heldtool} are {seaglidestorage[0].storageRoot}, and {seaglidestorage[1].storageRoot}");
                        }
            ...
            if (Player.main == null) {
                if (timer == 0)
                {
                    Logger.LogInfo("Player.main is null! It is likely that a save has not been loaded yet, if not, further debugging required");
                }
                return;
            }
